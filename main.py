...
def get_est(Img, label, model_num, t, iter=3):
    result = 0
    for _ in range(iter):
...
sampled_images = torch.load('../data/sampled_images2_label_'+str(label)+'.pt')
...
for t in range(499, -1, -1):
    est = get_est(sampled_images[time][index], label, model_num=0, t=t, iter=3).cpu().numpy()
    est_list.append(est)
    time += 1
    print("time", t, "finished.")

torch.save(est_list, '../output/PMI_label2_'+str(label)+'_index_'+str(index)+'.pt')
